...
module.exports = {
    ...
    try {
      ...
      if (customerMode === "existing" && !customerId) {
        throw new ApiError(
          ...
          "Customer ID is required for existing customer",
        );
      }
      ...
      if (customerMode === "new" && !newCustomerData) {
        throw new ApiError(
          httpStatus.BAD_REQUEST,
          "New customer data is required for new customer",
        );
      }
      ...
      if (customerMode === "new" && newCustomerData) {
        try {
          ...
          const customerResponse = await axios.post(
            `${process.env.V1_BASE_URL}/v1/mobile/staff/customer/create-customer`,
            newCustomerData,
            {
              headers: {
          ...
          if (customerRes?.data?._id) {
            ...
            throw new ApiError(
              httpStatus.BAD_REQUEST,
              "Failed to create customer: No customer ID returned",
            );
          }
        } catch (error) {
      ...
      if (newCustomerSelections && newCustomerSelections.length > 0) {
        const selectionPayload = {
          ...
          selectionData: newCustomerSelections.map((s, index) => {
            return {
              ...
              productVariantId: s.products.map((p) => ({
        ...
        const selectionResponse = await axios.post(
          `${process.env.V1_BASE_URL}/v1/mobile/staff/selection/create`,
          selectionPayload,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        if (selRes?.data) {
          const createdSelections = Array.isArray(selRes.data)
            ? selRes.data
            : [selRes.data];
      ...
      if (assignTo && assignTo.length > 0) {
        ...
        const assignResponse = await axios.put(
          `${process.env.V1_BASE_URL}/v1/mobile/staff/customer/assign-staff`,
          assignPayload,
          {
            headers: {
              "Content-Type": "application/json",
      ...
      for (const selectionId of finalSelectionIds) {
        ...
        if (selection.customerId.toString() !== finalCustomerId) {
          throw new ApiError(
            httpStatus.BAD_REQUEST,
            `Selection ${selectionId} does not belong to this customer`,
          );
        }
      ...
      const enrichedProducts = products.map((product) => {
        const isTempSelection =
          product.selectionId === null || !product.selectionId;
        ...
        if (isTempSelection && createdSelectionsData?.data) {
          const createdSelections = Array.isArray(createdSelectionsData.data)
            ? createdSelectionsData.data
            : [createdSelectionsData.data];
          ...
          if (createdSelections.length > 0) {
            ...
            if (createdSelection?.products) {
              const matchingCreatedProduct = createdSelection.products.find(
                (cp) =>
                  cp.p_id === product.productVariantId ||
                  cp.product_variant_id === product.productVariantId,
              );
        ...
        let metadata = {};
        if (selection) {
          const matchingProduct = selection.products?.find(
            (p) =>
          ...
          if (matchingProduct) {
            metadata = {
              productName:
                product.productName || matchingProduct.product_name || "",
              seriesName:
                product.seriesName ||
                (Array.isArray(matchingProduct.series_name)
                  ? matchingProduct.series_name.join(", ")
                  : matchingProduct.series_name || ""),
              dimension: matchingProduct.dimension || "",
              designCode:
                product.designCode || matchingProduct.design_code || "",
              variantId:
                product.variantId ||
                matchingProduct.variant_id?.toString() ||
                "",
              seriesId:
                product.seriesId || matchingProduct.series_id?.toString() || "",
              variantName:
                product.variantName || matchingProduct.variant_name || "",
            };
          }
        }
      ...
      for (const product of enrichedProducts) {
        const existingChallanProduct = await challanService.get({
          "products.selectionProductId":
            product.selectionProductId || product._id,
          deletedAt: null,
        });
      ...
      try {
        const challanNumber =
          await challanService.generateChallanWithTransaction(session);
        ...
        try {
          ...
          console.warn(
            "Could not update product flags in v1:",
            v1Error.message,
          );
        }
        ...
        if (transporterId && transporterAmount && transporterAmount > 0) {
          try {
            ...
            console.warn(
              "Could not create transporter history in v1:",
  ...
  getAllChallans: catchAsync(async (req, res) => {
    ...
    const pipeline = [
      ...
      // Add frontend-friendly fields
      {
        $addFields: {
          ...
          "products.totalSquareFeet": {
            $ifNull: ["$products.totalSquareFeet", 0],
          },
          ...
          "products.selectionName": {
            $ifNull: ["$products.selectionName", "N/A"],
          },
          "products.selectionProductId": {
            $ifNull: ["$products.selectionProductId", ""],
          },
      ...
      // Group back to restore original structure
      {
        $group: {
          ...
          transporterAmount: { $first: "$transporterAmount" },
    ...
    if (challans[0] && challans[0].results && challans[0].results.length > 0) {
      const resultsWithCustomer = await Promise.all(
        challans[0].results.map(async (challan) => {
          let customer = null;
          if (challan.customerId) {
            try {
              // Fetch customer from V1 like in getChallan
              customer = await v1Service.getCustomer(
                challan.customerId.toString(),
                req.headers.authorization,
              );
            } catch (error) {
              console.warn(
                `Could not fetch customer ${challan.customerId} from v1:`,
                error.message,
              );
            }
          }
  ...
  getChallan: catchAsync(async (req, res) => {
    const { id } = req.params;
    const result = await challanService.getChallanWithProducts(
      id,
      req.headers.authorization,
    );
  ...
  updateChallan: catchAsync(async (req, res) => {
    ...
    if (customerId) {
      const customer = await v1Service.getCustomer(
        customerId,
        req.headers.authorization,
      );
      if (!customer) {
    ...
    if (selectionIds) {
      ...
      for (const selectionId of selectionIds) {
        const selection = await v1Service.getSelection(
          selectionId,
          req.headers.authorization,
        );
    ...
    if (purchaseOrderId !== undefined)
      challan.purchaseOrderId = purchaseOrderId;
    ...
    if (status && challan.products && Array.isArray(challan.products)) {
      try {
        ...
        console.warn(
          "Could not update challan status in selection products:",
          v1Error.message,
        );
      }
    }
  ...
  deleteChallan: catchAsync(async (req, res) => {
    ...
    if (
      challan.transporterId &&
      challan.transporterAmount &&
      challan.transporterAmount > 0
    ) {
      try {
        ...
        console.warn(
          "Could not delete transporter history in v1:",
          historyError.message,
        );
  ...
  getCustomersWithChallans: catchAsync(async (req, res) => {
    ...
    await Promise.all(
      aggregatedCustomers.map(async (item) => {
        try {
          if (!item._id) return;
          const customer = await v1Service.getCustomer(
            item._id.toString(),
    ...
    customersWithDetails.sort(
      (a, b) => new Date(b.lastChallanDate) - new Date(a.lastChallanDate),
    );
